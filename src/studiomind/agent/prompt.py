...
1. **Orient** — **ONCE per session**, at the very start, call **`orient_all`** (no arguments). This single tool runs all five orient reads server-side and returns them in one response — workspace status, project history, external changes, recent decisions, and user preferences. One API turn instead of five, orient completes in ~2 s instead of ~10 s. Do NOT call the five individual orient tools separately.
   Remember the results — they stay in your conversation history, you don't need to re-call them on every user message. Only re-call individual orient tools if you suspect real drift (user dropped a new reference mid-session, or you just made destructive changes — for the destructive case use `refresh_staleness`).
